...
export default function CashBackManager({ currentUser }: CashBackManagerProps) {
  ...
  const triggerAppraisal = async (
    ...
    try {
      ...
      // Local fallback calculation logic (HS: 500P~2,000P max 3,000P)
      if (currentShareType === 'hs') {
        const basePts = 500;
        const confBonus = isConfidential ? 1000 : 200;
        const decBonus = decisionType === 'overturned' ? 500 : 200;
        const total = Math.min(3000, basePts + confBonus + decBonus);

        setAnalysisResult({
          appraisedPoints: total,
          scarcityGrade: isConfidential ? '신규 세번 (DB 미등재 신제품 회시)' : '일반 세번 (표준 분류 규격)',
          scarcityRate: isConfidential ? 90.0 : 75.0,
          matchedPublicCount: isConfidential ? 1 : 4,
          basePoints: basePts,
          confidentialBonus: confBonus,
          decisionBonus: decBonus,
          scarcityBonus: 0,
          appraisalSnippet: `본 품목분류 회시/결정문은 CUSWAY 마스터 DB 대조 결과 [${isConfidential ? '비공개 신규 세번' : '일반 세번'}]로 판정되어, ₩${total.toLocaleString()}P의 실무 마일리지가 산정되었습니다.`
        });
      } else {
        const basePts = 1500;
        ...
        setAnalysisResult({
          ...
          decisionBonus: decBonus,
          scarcityBonus: 0,
          appraisalSnippet: `본 조세심판원/관세평가 결정문은 CUSWAY 마스터 DB 대조 결과 독창성 ${isConfidential ? '95.5%' : '80.0%'}로 산정되어, 최대 ₩${total.toLocaleString()}P의 캐시백이 책정되었습니다.`
        });
      }
    } finally {
  ...
  const processUploadedFile = (file: File) => {
    ...
    setTimeout(() => {
      ...
      if (isValuationDoc) {
        ...
        setParseSuccessMsg(`📦 품목분류 사전심사 회시서/결정서 감지: 비공개 결정례 가치 감정 가동 (500P~3,000P)`);
      } else {
        ...
        setParseSuccessMsg(`📄 문서 파일 "${file.name}" 분석 완료! 비공개 결정례 가치 감정을 시작합니다.`);
      }

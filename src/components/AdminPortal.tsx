...
온라인 데모 및 무료 14일 Enterprise 파일럿 테스트를 신청해보세요!
  },
  {
    ...
    category: 'enterprise',
    title: '🚢 포워딩(국제물류) 업체 전용 화이트라벨 A4 리포트 솔루션 제안',
    badge: '포워더 B2B',
    description: '화주 응대용 HS코드/관세율 즉시 조회 및 포워딩사 로고가 찍힌 화이트라벨 A4 통관 리포트 제공 제안',
    subject: '[제안서] {companyName} 포워딩 전용 CUSWAY 화주 제출용 A4 통관 리포트 & HS코드 솔루션 안내',
    content: `[CUSWAY 포워딩/국제물류 전용 B2B 솔루션 제안]

안녕하세요, {companyName} {contactName} 물류/영업 담당자님.
대한민국 1등 관세 통관 AI 솔루션 CUSWAY입니다.

화주(수출입 기업)로부터 매일 "이 품목 HS코드가 뭐예요?", "관세 얼마 나와요?", "수입 요건 뭐 필요해요?"라는 질문을 받을 때마다 관세사에게 전화해 아쉬운 소리 하기 번거로우셨나요?

이제 CUSWAY를 도입하시면 포워딩 실무진이 3초 만에 4단계 통관 요건을 확인하고, [ {companyName} ] 상호와 로고가 선명하게 인쇄된 정규 A4 통관 심사 리포트(PDF)를 화주에게 바로 전달하실 수 있습니다.

🚢 포워딩 파트너사 주요 도입 혜택:
1. 3초 품목분류(HSK 10단위) 및 기본/WTO/FTA 최적 관세율 즉시 산출
2. 세관장확인 필수 법령(KC인증, 식약처 검역, 전파법 등) 사전 시뮬레이션
3. 화주 제출용 A4 리포트에 [ {companyName} ] 로고/직인 자동 합성 (화이트라벨)
4. 화주 견적서 송부 시 통관 전문 리포트 동봉으로 수주율 2배 극대화

현재 신용카드 등록 없이 30일간 Pro/Enterprise 전 기능(화이트라벨 리포트 무제한 출력) 무료 체험이 가능합니다.
👉 30일 무료 체험 바로가기: https://cusway.kr`
  },
  {

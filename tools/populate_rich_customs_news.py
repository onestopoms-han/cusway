...
■ 3. 시행일자: 2026년 9월 4일(금) 즉시 시행""",
        "attached_files": json.dumps([{"name": "20260904_품목분류_및_선택세율_적용지침_전문.pdf", "size": "215.4 KB"}, {"name": "농축수산물_세율적용_실무매뉴얼.pdf", "size": "340.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 2,
        "tag": "FTA 협정세율",
...
3. 영세 중소기업 대상 FTA 사후적용(1년 이내) 경정청구 간이 환급 지원 체계 확대""",
        "attached_files": json.dumps([{"name": "20260904_한EU_RCEP_원산지증명_개정고시문.pdf", "size": "188.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 3,
        "tag": "통합공고 요건",
...
- 최초 수입 정밀검사 대상 품목의 검사 진행 상황 모바일 실시간 알림 서비스 개시""",
        "attached_files": json.dumps([{"name": "식약처_관세청_실시간_검역승인_연계_운영매뉴얼.pdf", "size": "420.5 KB"}], ensure_ascii=False)
    },
    {
        "id": 4,
        "tag": "관세평가",
...
다국적기업 본·지사 간 특수관계 거래 시 과세가격 결정(제1방법 배제 여부) 및 라이선스 계약에 따른 로열티 가산 판단 기준을 집대성한 2026년도 최신 판례집을 발간합니다.""",
        "attached_files": json.dumps([{"name": "2026_관세평가_이전가격_심사사례집.pdf", "size": "1.2 MB"}], ensure_ascii=False)
    },
    {
        "id": 5,
        "tag": "특송 통관",
...
전자상거래 특송물품 수입신고 시 명의도용 피해를 방지하기 위해 관세청 국민비서 또는 카카오톡 기반 2단계 인증이 필수 적용됩니다.""",
        "attached_files": json.dumps([{"name": "해외직구_본인인증_개편안내문.pdf", "size": "150.2 KB"}], ensure_ascii=False)
    },
    {
        "id": 6,
        "tag": "관세청 고시",
...
3. 수입신고 시 할당관세 추천서(추천기관: 한국석유화학협회, 농림축산식품부 등) 전자 제출 필수""",
        "attached_files": json.dumps([{"name": "2026_하반기_할당관세_품목_및_세율표.pdf", "size": "450.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 7,
        "tag": "WCO 품목분류",
...
WCO HSC에서 최종 확정된 첨단 신산업 물품의 HS Code 분류 기준을 국내 세관 사전심사 및 통관 심사에 즉시 적용합니다.""",
        "attached_files": json.dumps([{"name": "WCO_HSC_73차_분류결정서_국문번역본.pdf", "size": "620.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 8,
        "tag": "FTA 협정세율",
...
칠레 발 한국행 화물이 미국 또는 중남미 항구를 경유할 경우, 선화증권(Through B/L) 제출 시 세관 통제 하 환적 입증을 자동 인정합니다.""",
        "attached_files": json.dumps([{"name": "한칠레_직접운송_간소화_지침.pdf", "size": "180.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 9,
        "tag": "세관 심사기법",
...
기업심사 대상 업체는 사전 자율점검표를 활용하여 수입신고 적정성을 검토하시기 바랍니다.""",
        "attached_files": json.dumps([{"name": "2026_기업심사_자율점검표.pdf", "size": "310.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 10,
        "tag": "수출입 물류",
...
비정형 은닉 물품 판독률이 95% 이상으로 향상되어 정상 화물의 입항 후 통관 소요 시간이 3시간 이내로 단축됩니다.""",
        "attached_files": json.dumps([{"name": "스마트통관_AI_XRay_운영계획.pdf", "size": "512.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 11,
        "tag": "관세청 공고",
...
관세법령정보포털(CLIP) 및 CUSWAY 지식허브 DB에 120건의 최신 사전회시 사례가 실시간 동기화되었습니다.""",
        "attached_files": json.dumps([{"name": "2026_3차_품목분류_사전심사_결정사례집.pdf", "size": "2.4 MB"}], ensure_ascii=False)
    },
    {
        "id": 12,
        "tag": "FTA 협정세율",
...
부가가치기준(RVC) 및 세번변경기준(CTH) 충족을 증명하는 원자재 원산지확인서 보관 필수 (5년 보존).""",
        "attached_files": json.dumps([{"name": "인도네시아_CEPA_원산지검증_대응매뉴얼.pdf", "size": "480.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 13,
        "tag": "관세환급",
...
수출금액 10,000원당 정액 환급액이 품목별 평균 12% 인상되어 서류 없는 간이 환급 혜택이 확대됩니다.""",
        "attached_files": json.dumps([{"name": "20260815_간이정액환급률표_개정본.pdf", "size": "390.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 14,
        "tag": "대외무역법",
...
원산지 미표시 및 허위표시 시 대외무역법 제33조에 의거 최대 3억원의 과징금 또는 형사고발 조치됩니다.""",
        "attached_files": json.dumps([{"name": "원산지표시_적정성_자가진단_체크리스트.pdf", "size": "210.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 15,
        "tag": "통합공고 요건",
...
제28류, 29류, 38류 수입 화학제품 통관 시 화학물질 수입신고서 및 유독물질 수입허가증 첨부 필수.""",
        "attached_files": json.dumps([{"name": "2026_신규_세관장확인_화학물질_목록.pdf", "size": "320.0 KB"}], ensure_ascii=False)
    },
    {
        "id": 16,
        "tag": "관세청 속보",
...
수입신고서 전송 후 수리 필증 교부까지 평균 소요시간이 15분 이내로 단축되었습니다.""",
        "attached_files": json.dumps([{"name": "차세대_유니패스_이용자_가이드.pdf", "size": "670.0 KB"}], ensure_ascii=False)
    }
]

print("1. Re-populating customs_news table in cusway.db with 16 rich cumulative records...")
cur.execute("DELETE FROM customs_news")

for item in RICH_NEWS:
    cur.execute("""
        INSERT INTO customs_news (id, tag, title, date, agency, summary, link, full_content, attached_files)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        item["id"], item["tag"], item["title"], item["date"], item["agency"],
        item["summary"], item["link"], item["full_content"], item["attached_files"]
    ))

conn.commit()
print("Successfully inserted rich cumulative records.")

cur.execute("SELECT COUNT(*) FROM customs_news")
print("Total rows in customs_news now:", cur.fetchone()[0])

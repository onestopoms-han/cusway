...
FINAL_3_IDS = [20, 27, 41]
FINAL_3 = [item for item in NEW_50_RELEASE_ITEMS if item["id"] in FINAL_3_IDS]

def verify_final_3():
    db = SessionLocal()
    print("=" * 80)
    print(f"🔬 최종 잔여 3개 품목 정밀 검증 시작 (총 {len(FINAL_3)}건)")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"📊 최종 3개 검증 결과: 통과 {passed}/{len(FINAL_3)}건 ({passed/len(FINAL_3)*100:.1f}%)")
    print("=" * 80)
    db.close()
    return passed, len(FINAL_3), failed

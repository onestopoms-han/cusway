...
⚠️ [통칙 제1호 및 제3호에 따른 완제품 성상(Subject) vs 배합 원재료(Ingredients) 엄격 분리 대원칙]:
1. [원재료 성분 낚임 방지 (화장품 vs 화학품/의료용품)]:
   - 수입 물품이 앰플, 에센스, 크림, 로션, 세럼, 스킨 등 피부 도포용 완제품인 경우, 성분표에 '글리세린', '히알루론산', '비타민', '정제수' 등이 기재되어 있더라도 절대로 순수 화학물질(제29류 제2905.45호 등)이나 의료용 필러/주사제(제30류 제3006.70호)로 오분류해서는 안 됩니다. (제29류 주 제1호: 단일 화학물이 아닌 조제품은 제29류 배제 ➔ 제3304호 제3304.99-1000호 기초화장용 제품류로 분류).
2. [복합 물품의 본질적 특성(GRI 3(b)) 적용 (완성차/의류 vs 부품)]:
   - '선풍기 내장 조끼'는 팬/모터(제8414호)가 아닌 착용용 의류(제6201호/제6202호)로 분류합니다.
   - '전기자전거(E-Bike)'는 구동 모터(제8501호)나 자전거 부품(제8714호)이 아닌 완성된 전동 이륜차(제8711.60-1000호)로 분류합니다.
3. [통칙 제1호에 따른 수입 당시 본질(As-Is State) 판정]:
   - '주요 용도 및 기능' 항목에 가공 후 완제품(예: 빵/과자 제조용, 주스 착즙 원료, 자동차 부품 조립용, 비타민 건강기능식품용 등)이 기재되어 있더라도, 수입 당시 대상 물품의 물리적 상태(예: 냉동 과실 0811호, 미가공 종실 1207호, 철강제 볼트 7318호 등)를 덮어써서 최종 가공품 세번(주스 2009호, 빵 1905호, 자동차 8708호 등)으로 왜곡 분류해서는 절대 안 됩니다. 관세율표는 '수입 통관 시점의 물품 자체 상태'를 기준으로 판정해야 합니다.
...
[작성 및 판정 지침 (관세평가분류원 수석 심사관급 심층 소명서 작성 원칙)]
1. recommendedHsCode: 10자리 세번 코드를 정확하게 명시하십시오. (예: 1211.20-2120)
2. appliedGris: 분류 시 핵심 근거가 된 관세율표 해석에 관한 일반통칙 번호(예: 통칙 제1호, 통칙 제3호 나목, 통칙 제6호)들을 배열로 반환하십시오.
3. legalReasoning: 관세청 품목분류 사전심사 결정서 및 관세법 소명서 수준으로 최고도의 전문성과 법리적 명쾌함을 갖추어 작성하십시오. 반드시 아래의 4대 단락 구조로 명확하게 번호를 매겨 서술하십시오:
   가. [물품 개요 및 물리적 성상 분석]: 대상 물품의 물리적 상태(원형/건조/분말/캡슐/액상/조제품 등), 구성 성분비(%), 가공도(단순 건조, 분쇄, 볶음, 추출, 배합 등)를 정확히 특정하고 As-Is 수입 통관 시점의 본질적 상태를 분석합니다.
   나. [관세율표 부/류 주(Note) 및 WCO 해설서 규정 검토]: 본 호의 표제 용어와 함께, 해당 부/류의 주규정 및 WCO 해설서 총설의 포함 기준과 배제 규정(Exclusion Notes)을 원문 그대로 인용하여 적합성을 정밀 검토합니다.
   다. [일반통칙(GRI 제1호~제6호) 단계별 순차 적용 논리]: 통칙 제1호(호의 용어 및 부/류의 주), 제3호(혼합물/복합물의 본질적 특성), 제6호(소호 분류)를 순차적으로 적용하여 4단위 호, 6단위 소호, 10단위 HSK 세번으로 좁혀가는 법리적 논증 과정을 명시합니다.
   라. [최종 세번 확정 및 경합 세번 배제 소명]: 최종 세번을 명확히 제시하고, 유사하거나 혼동하기 쉬운 타 경합 세번(예: 제2106호 vs 제1211호, 제2202호 vs 제2101호 등)이 배제되는 구체적인 법적 근거(왜 해당 세번이 부적합한지)를 명쾌하게 소명합니다.
4. sectionNote & chapterNote: 부의 주(Section Note) 및 류의 주(Chapter Note) 규정 중 본 품목과 관계된 실제 구절(인용구) 또는 조항을 원문에서 정확하게 찾아 명시하십시오. (예: '제12류 총설 및 제1211호 해설서에 따라...')
...
7. competingHsCodes: 최종 분류로 고려되었으나 탈락한 경쟁/경합 HS Code(1~2개)의 번호, 명칭, 배제 이유를 명확하게 작성하십시오.

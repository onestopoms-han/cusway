...
// 2. Clear/Initialize target file
...
for (const file of appendFiles) {
  ...
  try {
    ...
    // Extract the content between const data = ` and `;
    ...
    if (startIndex !== -1 && endIndex !== -1 && endIndex > startIndex) {
      const dataContent = fileContent.substring(startIndex + startToken.length, endIndex);
      const dataDecl = fileContent.substring(startIndex, endIndex + endToken.length);
      let dataVal = '';
      eval(dataDecl + '; dataVal = data;');
      fs.appendFileSync(targetPath, dataVal, 'utf8');
    } else {
